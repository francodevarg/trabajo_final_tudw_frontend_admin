...
<template>
  <div class="mb-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center gap-4 mb-4">
      ...
      <div class="sm:ml-auto w-full sm:w-auto">
        <div class="relative w-full sm:w-80 lg:w-96">
          ...
          <input v-model="search" type="text" placeholder="Buscar por nombre, email o matrícula..."
            class="input h-9 w-full !pl-10 pr-9 text-sm" />

          <button v-if="search" class="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600"
            @click="search = ''">
            <X class="size-4" />
          </button>
        </div>
    ...
    <div class="flex items-center gap-2 flex-wrap">
      <select v-model="specialtyFilter" class="input !w-auto text-xs h-10 pr-7">
        <option value="">Todas las especialidades</option>
        <option v-for="s in specialtiesStore.items" :key="s.id" :value="s.name">{{ s.name }}</option>
      </select>

      <select v-model="statusFilter" class="input !w-auto text-xs h-10 pr-7">
        <option v-for="opt in STATUS_OPTIONS" :key="opt.value" :value="opt.value">{{ opt.label }}</option>
      </select>

      <select v-model="insuranceFilter" class="input !w-auto text-xs h-10 pr-7">
        <option value="">Todas las obras sociales</option>
        <option v-for="ins in insurancesStore.items" :key="ins.id" :value="ins.name">{{ ins.name }}</option>
      </select>

      <button v-permission="'doctor.add_doctor'" class="btn btn-primary" @click="$emit('create')">
        <Plus class="size-4 mr-1" />
        Nuevo Médico
      </button>
      <button v-if="hasFilters" class="text-xs text-slate-400 hover:text-slate-600 transition px-2 py-1"
        @click="clearFilters">
        Limpiar filtros
      </button>
